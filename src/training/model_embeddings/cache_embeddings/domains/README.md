...
### "Domain config not found" / unknown domain

The domain must exist as a key under `domains:` in `prompts.yaml` (or as a
supported alias: `medical` → `health`, `programming` → `computer_science`).
Check:

```bash
python3 -c "import yaml; print(sorted(yaml.safe_load(open('domains/prompts.yaml'))['domains']))"
```
